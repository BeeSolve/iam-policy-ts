...
- [x] 1. Create naming utilities and service file renderer
  - [x] 1.1 Create `scripts/naming.ts` with `prefixToCamelCase` and `prefixToPascalCase` functions
  ...
  - [x] 1.2 Create `scripts/render-service-file.ts` with the service file renderer
  ...
  - [x] 1.3 Create `scripts/render-meta-file.ts` with the metadata file renderer
  ...
  - [x] 1.4 Create `scripts/render-barrel-file.ts` with the barrel file renderer
  ...
  - [x] 1.5 Write unit tests for naming utilities
...
- [x] 2. Rewrite the generator script
  - [x] 2.1 Rewrite `scripts/generate-catalog.ts` to produce per-service file structure
  ...
  - [x] 2.2 Implement stale file removal in the generator
  ...
  - [x] 2.3 Implement `package.json` exports map update in the generator
  ...
  - [x] 2.4 Write property test for service function identity (Property 1)
  ...
  - [x] 2.5 Write property test for file structure and naming (Property 2)
  ...
  - [x] 2.6 Write property test for one-to-one file mapping (Property 3)
...
- [x] 3. Checkpoint - Generator produces valid output
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Update package entry and remove legacy modules
  - [x] 4.1 Update `src/index.ts` to use new catalog barrel
  ...
  - [x] 4.2 Delete `src/helpers.ts` and `src/helpers.test.ts`
    - Remove the legacy helper module entirely
    - _Requirements: 6.3_

  - [x] 4.3 Delete `src/catalog.ts`
    - Remove the monolithic catalog file (replaced by `src/catalog/` directory)
    - _Requirements: 1.1_

  - [x] 4.4 Write property test for barrel purity and completeness (Property 4)
  ...
  - [x] 4.5 Write property test for exports map completeness (Property 5)
...
- [x] 5. Update render.ts to use per-service functions
  - [x] 5.1 Update `src/render.ts` import and `renderActionString` function
  ...
  - [x] 5.2 Update `src/render.test.ts` to expect new output format
  ...
  - [x] 5.3 Write property test for stale file removal (Property 6)
  ...
  - [x] 5.4 Write property test for ESM specifier compliance (Property 7)
    - **Property 7: ESM specifier compliance**
    - For any generated file, verify all import/export specifiers end with `.js`
    - **Validates: Requirements 7.5**

- [x] 6. Checkpoint - Build and tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Run generator and verify build
  - [x] 7.1 Run `npm run generate` to produce the new catalog structure
  ...
  - [x] 7.2 Run `npm run build` and verify zero TypeScript errors
